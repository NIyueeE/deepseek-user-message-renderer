...
- **助手消息 raw / 渲染切换**:每条助手回复的操作栏里(复制按钮旁)会注入一个
  ...
  并且在宿主重新渲染后仍保持一致。该按钮直接克隆相邻的原生操作按钮并放在操作栏
  **最右侧**,因此自带宿主的 hover/active/focus 样式;图标改为「折角文件 + `</>`」的
  描边轮廓(lucide `file-code-corner`),几何数据硬编码在脚本内、绘制在原生图标的
  尺寸框里,不依赖任何 CDN 或图标站;提示气泡用页面自己的 tooltip 变量绘制,而非
  浏览器默认的 title 小方框,并且写明点击后会做什么——渲染视图下是 `显示源码`,
  raw 视图下是 `显示预览`。raw 原文按 DeepSeek 渲染任意围栏代码块的方式呈现:
...
- **代码块重建为 DeepSeek 官方 `md-code-block` 结构**:带语言标签的横幅、
  ...
  **每个**代码块都会重建该结构,而不只是 highlight.js 能着色的那些:
  `text` / `mermaid` 之类的围栏同样会得到原生外壳。
- **代码块内硬换行完整保留**;未知语言(如 `mermaid`)保持为干净的代码块,
  不会在控制台产生警告。
- **扫描会收敛**:只有当消息确实需要处理(尚未渲染、文本被原地改写、主题变化)
  或助手操作栏 / 自己的切换按钮出现时,观察器才会安排一次扫描。已经正确的 raw 视图
  不会被反复重建,因此打开的视图不会在微任务队列里自我循环、把页面的定时器饿死。
...
- [`test/assistant-raw.test.ts`](test/assistant-raw.test.ts):助手消息的
  raw / 渲染切换——按钮注入到复制按钮所在行、默认渲染状态、raw 视图的原生代码块外壳、
  无损切换、多次扫描下的幂等(含回归守卫:打开的视图绝不被反复重建、页面定时器不被饿死)、
  宿主重新渲染后自动补回、各消息互不影响,以及用户消息不被触碰。
- [`test/code-block.test.ts`](test/code-block.test.ts):在 highlight.js 完全没有
  加载的页面(CDN `@require` 失败)上代码块仍会得到原生外壳;未知/纯文本语言保持原文
  不被改动;浅色/深色变体跟随页面的 `data-ds-dark-theme` 标记;主题切换会重建 raw
  视图,而不会留下过期的明暗变体。
