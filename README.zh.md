...
- **助手消息 raw / 渲染切换**:每条助手回复的操作栏里(复制按钮旁)会注入一个
  ...
  **最右侧**,因此自带宿主的 hover/active/focus 样式;`</>` 图标的尺寸、线宽与
  ...
  绘制,而非浏览器默认的 title 小方框。raw 原文则使用 DeepSeek 自己的设计变量与 markdown 容器类渲染,
  并采用页面的**正文字体而非等宽字体**——DeepSeek 自己的 raw 气泡就是以 `pre-wrap` + 16px/24px
  的正常字体呈现的,此前用代码字体是我的自创设计,会让两种视图看起来像两个产品。配色自动跟随明暗主题。
- **LaTeX 公式**(KaTeX):`$...$`、`$$...$$`、`\(...\)`、`\[...\]`。
